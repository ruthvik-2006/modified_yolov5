...
__device__ void apply_conv_block(const vector<float>& input, vector<float>& output,
    ...
    convolution_kernel(conv_tmp_output.data(), input.data(), weights.data(), bias.data(),
                      N, C_in, H_in, W_in,
                      C_out, H_out, W_out,
    ...
    batchnorm2d(bn_tmp_output.data(), conv_tmp_output.data(), gamma.data(), beta.data(),
                      mean.data(), var.data(), N, C_out, H_out, W_out, eps);

    output.resize(bn_tmp_output.size());
    Silu(bn_tmp_output.data(), output.data(), bn_tmp_output.size());
}



__global__ void apply_bottleneck_block(const vector<float>& input, vector<float>& output,
                            int N, int C_in, int H_in, int W_in,
                            bool shortcut, int g) {
...
__global__ void apply_sppf_block(const vector<float>& input, vector<float>& output,
                      int N, int C_in, int H_in, int W_in,
